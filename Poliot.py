...
pygame.display.set_caption("Пилот")
...
meteor_imges=[]
meteor_list = ['meteorBrown_big1.png','meteorBrown_big4.png','meteorBrown_med1.png','meteorBrown_med3.png',
...
def draw_shield_bar(surf, x, y, pct):
    ...
    if pct < 30:
        colr =  RED
    pygame.draw.rect(surf, colr, fill_rect)
...
while running:
    ...
    for hit in hits:
        ...
        #print(hit.radius, ' - shield')
        if player.shield <= 0:
            de = Explosion(player.rect.center, 'player')
            all_sprites.add(de)
            #running = False
            player.hide()
            player.lives -= 1
            player.shield = 100
        newmob()
    if not player.lives and not de.alive():
        running = False
